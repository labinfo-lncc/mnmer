...
\bioctitle[mnmer]{(m,n)-mer - A Simple and New Statistical Feature for viral Classification}
%% also: \bioctitle{Title used for both header and title page}
%% or... \title{Title used for both header and title page}
\author[1]{Amanda Araújo Serrão de Andrade} 
\author[2]{Marco Grivet}
\author[1]{Otávio Brustolini}
\author[1]{Ana Tereza Ribeiro de Vasconcelos
\thanks{\email{atrv@lncc.br}}}
\affil[1]{National Laboratory for Scientific Computing, Bioinformatics Laboratory (LABINFO), Petrópolis, Rio de Janeiro, Brazil}
\affil[2]{Pontifícia Universidade Católica do Rio de Janeiro, 22451-900, Brazil}
...
\packageVersion{\Sexpr{BiocStyle::pkg_ver("mnmer")}}
\newpage
...
For classification outside of the mnmer program, we utilize the (1,1)-mer feature matrices. Here's a real-world example of code using \CRANpkg{Caret}, \CRANpkg{MLeval} and \CRANpkg{data.table}:
...
\begin{figure*}
\includegraphics[keepaspectratio,width=12cm]{\jobname-Fig6.png}
\raggedright
\caption{\label{fig:wide}ROC curve and Area Under the Curve Value}
\end{figure*}

\subsection{What to expect from newest versions}

We would like to add new functionalities related to handle big multiFASTA files, paralelism and compressed files. 
We are also implementing the option to generate the (m,n)-mer distributions for IUPAC and N bases as well.
